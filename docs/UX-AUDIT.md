...
| U11 Autosave ordering | Done, indexing state deferred | Save status follows the newest request only; knowledge entry editors are controlled and refresh their own entry; workspace settings merge under a row lock. Settings and entries carry a revision: a save over somebody else's newer change is refused and the page shows their version (`e2e/revisions.spec.ts`). An "indexed and ready" state is not built. |
