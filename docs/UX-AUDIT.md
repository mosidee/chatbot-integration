...
| U05 Connection freshness | Done | One socket in the shell with a status pill, refetch on reconnect (except Settings, which keeps the version it shows), stop on 4401/4403, workspace status refreshes the session. Server re-validates live sockets (finding 18). |
...
| U11 Autosave ordering | Done, indexing state deferred | Save status follows the newest request only; knowledge entry editors are controlled and refresh their own entry; workspace settings merge under a row lock. Settings and entries carry a revision: a save over somebody else's newer change is refused and the page shows their version (`e2e/revisions.spec.ts`); a settings save is judged only on the settings it changes, so an edit to another card is not a conflict (`apps/api/test/revisions.test.ts`). An "indexed and ready" state is not built. |
