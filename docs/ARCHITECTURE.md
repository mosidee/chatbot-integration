...
## Telling agents (ADR 0010)
`notify_agents` does two things: a realtime `conversation.updated` to open consoles after the commit, and, where VAPID keys are set, a `push` outbox row in the same transaction (under a savepoint). The push job re-reads the conversation, drops the notification if nobody is owed anything any more, picks recipients from the current membership (the colleague holding it for a customer message in `human` mode, otherwise every agent and admin), and posts an encrypted Web Push to each subscribed device of theirs (`push_subscriptions`, per workspace and device). It fires on a handoff, the waiting-human timer, a draft ready for approval, and a customer message in `human` or `waiting_human`. The console registers `/sw.js`, which only shows notifications, sets the app-icon count and opens the conversation on a tap; it caches nothing.
...
Editing knowledge is guarded twice. An entry's `updatedAt` is its revision: a save that started from an older one is refused with 409. A settings save sends the shown value of each setting it changes (`expected`) and is refused only when one of those changed, naming it. And a reindex re-checks under a row lock before it swaps chunks: `indexEntry` stores nothing if the entry changed while it was embedding, and `replaceFileSource` locks the source so two reindexes of one file cannot both insert.
...
Ordered in SQL, and nothing re-sorts it in the browser. Three groups, by who owns the *customer*: yours, then nobody's, then somebody else's. Inside each, longest wait first, where waiting means either the conversation was handed to a person (`waiting_human_since`) or the customer spoke last and nobody has answered (`last_customer_message_at >= last_message_at`, where an agent's reply moves `last_message_at` as the customer's and the AI's do); anything nobody is waiting on falls to the bottom of its group, newest first.
...
The worker schedules a pass every 15 minutes. With no workspace in the job it plans: one job per active workspace, keyed `idle-resolve-<workspace>-<slot>` so a restart cannot run a slot twice. Per workspace, with `autoResolveAfterHours` set, it finds conversations that are open, in `ai` mode, whose last non-event message came from the AI or a colleague — not the customer, and not a system holding message — and is older than the cutoff. Who spoke last is read from `messages`, not `last_message_at`, which says when but not who (AI, colleague or a system holding message).
