...
describe('fetching LINE media', () => {
  test('asks the content endpoint with the token, and can be cancelled', async () => {
    const seen: { url: string; auth: string | null; signal: AbortSignal | null }[] = []
    globalThis.fetch = (async (input: string | URL | Request, init?: RequestInit) => {
      seen.push({
        url: String(input),
    ...
    const fetchMedia = lineChannelAdapter.fetchMedia
    if (!fetchMedia) throw new Error('LINE fetches media')
    const media = await fetchMedia('line:5551234', LINE, { signal: controller.signal })

    expect(seen).toEqual([
      {
        url: 'https://api-data.line.me/v2/bot/message/5551234/content',
        auth: `Bearer ${LINE.channelAccessToken}`,
  ...
  test('reports a refusal instead of storing it as a photo', async () => {
    globalThis.fetch = (async () =>
      new Response('gone', { status: 404 })) as unknown as typeof fetch
    const fetchMedia = lineChannelAdapter.fetchMedia
    if (!fetchMedia) throw new Error('LINE fetches media')
    await expect(fetchMedia('line:1', LINE)).rejects.toThrow('404')
  })
})
