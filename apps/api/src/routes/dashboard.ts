import { loadDashboard, loadWorkspaceSettings } from '@ci/infra'
...
export function dashboardRoutes(ctx: ApiContext) {
  return new Elysia({ prefix: '/dashboard' }).use(authPlugin(ctx)).get(
    '/',
    async ({ workspaceId, query }) => {
      const settings = await loadWorkspaceSettings(ctx.db, workspaceId)
      return loadDashboard(ctx.db, {
        workspaceId,
        days: query.days ?? 14,
